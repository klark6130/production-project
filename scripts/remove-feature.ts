...
const getAttributeNodeByName = (
    ...
    return jsxAttributes.find(node => {
        // console.log('node', node.getNameNode().getNodeProperty('escapedText'))
        return node.getNameNode().getNodeProperty('escapedText') === name
    });
};

const getReplacedComponent = (attribute?: JsxAttribute) => {
    const value =  attribute?.getFirstDescendantByKind(SyntaxKind.JsxExpression)
    ?.getExpression()
    ?.getText();
