...
const ArticleRating = memo(({ className, articleId }: ArticleRatingProps) => {
    ...
    const {
        data,
        isLoading,
        isError: isErrorArticleRating,
        error: errorArticleRating,
    } = useArticleRating({ articleId, userId: userData?.id ?? '' });
    ...
    if (isErrorArticleRating) {
        ...
        return (
            <Text
                ...
                text={String(errorArticleRating)}
            />
        );
    }
    ...
    return (
        <RatingCard
            ...
            feedbackTitle={t(
                'Оставьте свой отзыв от статье, это поможет улучшить качество',
            )}
            hasFeedback
        />
    );
